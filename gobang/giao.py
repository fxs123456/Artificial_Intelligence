...
def is_GameOver(list_now):
    for c in range(COL):
        for r in range(ROW):
            if r < ROW - 4 and (r, c) in list_now and (r + 1, c) in list_now and (r + 2, c) in list_now and (
                    r + 3, c) in list_now and (r + 4, c) in list_now:
                return True
            elif c < COL - 4 and (r, c) in list_now and (r, c + 1) in list_now and (r, c + 2) in list_now and (
                    r, c + 3) in list_now and (r, c + 4) in list_now:
                return True
            elif r < ROW - 4 and c < COL - 4 and (r, c) in list_now and (r + 1, c + 1) in list_now and (
                    r + 2, c + 2) in list_now and (r + 3, c + 3) in list_now and (r + 4, c + 4) in list_now:
                return True
            elif r > 3 and c < COL - 4 and (r, c) in list_now and (r - 1, c + 1) in list_now and (
                    r - 2, c + 2) in list_now and (r - 3, c + 3) in list_now and (r - 4, c + 4) in list_now:
                return True
    return False
            ...
            if temp_shape5 == shape or temp_shape6 == shape:
                if score > max_score[0]:
                    max_score = (score, ((r + (0 + noffset) * x_direction, c + (0 + noffset) * y_direction),
                                         ...
                                         (r + (4 + noffset) * x_direction, c + (4 + noffset) * y_direction)),
...
# 画棋盘
def GobangWin():
    gw = GraphWin('giao', GRID_WIDTH * COL, GRID_WIDTH * ROW)
    gw.setBackground('gray')
    for j in range(0, GRID_WIDTH * COL + 1, GRID_WIDTH):
        l = Line(Point(j, 0), Point(j, GRID_WIDTH * COL))
        l.draw(gw)
    ...
    while is_game:
        if step_count % 2:
            ...
            if p_ai in aime_list:
                message = Text(Point(300, 500), 'AI gets a wrong next step.')
            ...
            if not ((x, y) in aime_list):
                ...
                step_count += 1
    # 游戏结束后的处理
    message = Text(Point(300, 500), 'Click anywhere to quit.')
    message.setStyle('bold')
    message.setSize(20)
    message.draw(gw)
    gw.getMouse()
